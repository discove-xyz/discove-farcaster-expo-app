  ...
  return structuredCast.serializedContent
    ...
    .flatMap((part, i): StructuredCastUnit[] => {
      ...
      if (!e) {
        ...
        return [
          ...(e[1] !== ""
            ? ([{ type: "plaintext", serializedContent: e[1] }] as const)
            : []),
          { type: "textcut", serializedContent: `${e[2]}.${e[4]}`, url: url },
          ...(e[5] !== ""
            ? ([{ type: "plaintext", serializedContent: e[5] }] as const)
            : []),
        ];
      }
    });
...
export function convertCastPlainTextToStructured({
  ...
  text: string;
  options?: { hideDoubleBracketContent?: boolean };
}): StructuredCastUnit[] {
  let structuredCast: StructuredCastUnit[] = [
    { type: "plaintext", serializedContent: text },
  ];

  if (options.hideDoubleBracketContent)
    structuredCast = structuredCast.flatMap((x) => extractDoubleBrackets(x));
